...
app.get("/id/:type", (req, res) => {
  const typ = parseInt(req.params.type);
  while (typ >= 1) {
    res.send(jokes[typ - 1]);
  }
});
...
app.patch("/jokes/:id", (req, res) => {
  ...
  const indexsearch = jokes.findIndex((joke) => joke.id === id);
  jokes[indexsearch] = replacejoke;
...
app.delete("/jokes/:id", (req, res) => {
  ...
  const indexsearch = jokes.findIndex((joke) => joke.id === id);
  if (indexsearch > -1) {
    jokes.splice(indexsearch, 1);
    res.sendStatus(200);
  }
  else {
    res.send({ error: `Joke with id ${id}, 404 Not Found. No jokes were deleted` })
  };
});
...
app.delete("/all", (req, res) => {
  ...
  if (key === masterKey) {
    ...
    res
      .status(404)
      .json({ error: `You're Not Authorize to perform this task.` });
  };
});
